...
// One day of the Spec §4 wire contract, decoded.
struct OutlookDay {
...
// Decoded outlook.json (Spec §4). `count` is 0..3; `fetched` is the
...
// NVS-backed cache (Spec §3 "camada 2"): survives reboot / power cut, so
// the CYD keeps showing the last known forecast when the internet is down.
...
// Result of resolving a cached forecast against the current date (Spec
// §5). `count` is the number of cache entries whose date is strictly
// after `todayDate` (i.e. "tomorrow" and beyond), in cache order.
struct ResolvedForecast {
    bool       degraded;     // true: no future date in the cache — show the degraded state
    uint32_t   cacheAgeSec;  // now - cache.fetched, for the "prev. há Xh" freshness indicator
    OutlookDay days[3];
    uint8_t    count;
};

// `todayDate` is "YYYY-MM-DD" in the same local zone the outlook.json
// dates are in (Spec §9: America/Sao_Paulo) — the caller derives it from
// the resolved wall clock (NTP-synced, or coasting on the ESP32 timer
// once synced once; Spec §5). `nowEpoch` is UTC epoch seconds
// (`time(nullptr)`), used only for the cache-age calculation.
//
